...
- (NSURL *)addParams:(NSDictionary *)params {
    ...
    NSString *subString = [_add substringToIndex:[_add length] - 1];
    return [NSURL URLWithString:[NSString stringWithFormat:@"%@%@", self.absoluteString,subString]];
}

- (NSDictionary *)params {
    NSMutableDictionary* pairs = [NSMutableDictionary dictionary];
    if (NSNotFound != [self.absoluteString rangeOfString:@"?"].location) {
        NSUInteger index = [self.absoluteString rangeOfString:@"?"].location + 1;
        NSString *paramString = [self.absoluteString substringFromIndex:index];
